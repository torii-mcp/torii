...
| `torii policy edit <tool> [target] [--create] [--copy-shared]` | abre a política no editor, valida e só então substitui |
...
`policy edit <tool> <target>` exige um alias já configurado. Se aquele alias ainda não tiver política própria, ele usa a compartilhada inteira, e criar uma exige `--create` — porque a política do target **substitui os accepts** do provider naquele alias, enquanto os denies compartilhados continuam valendo e não podem ser removidos ali. `policy show <tool> <target>` imprime os dois arquivos e o conjunto efetivo.

`--create` sozinho começa a política do alias **vazia**, para um alias que precisa ser mais restrito que a raiz e listar explicitamente o que passa. `--create --copy-shared` começa com uma **cópia da política compartilhada**, para um alias que precisa ser mais frouxo: o editor abre já com tudo dentro e você acrescenta. A cópia é um retrato, não um vínculo — dali em diante um accept novo na compartilhada não chega mais naquele alias, e o cabeçalho do arquivo gerado diz isso. `--copy-shared` sem `--create` é erro. `policy show` aceita os mesmos escopos e avisa quando um accept fica abaixo de `minimum_accept_tokens` e, por isso, é ignorado na avaliação.
